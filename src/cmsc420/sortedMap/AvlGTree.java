...
public class AvlGTree<K, V> extends AbstractMap<K, V> implements
    ...
    public V remove(Object key) {    	
        if (this.containsKey(key)) {
        	...
        	size--;
        	if (res.parent != null)
        		fixAfterModification(res.parent);
        	return res.value;
        } else 
        	return null;
    }
    
    public void print(AvlNode<K, V> cur) { 
    	if (cur != null) { 
    ...
    static final class AvlNode<K, V> implements Map.Entry<K, V> {
        ...
        public AvlNode<K, V> delete(AvlNode<K, V> node) {
        	int cmp = compare(node.key, this.key);
        	
        	debug("removing node "+node);
        	debug("checking node "+this+" cmp "+cmp);
        	debug("children = left "+left+" right = "+right);
        	...
        	if (cmp < 0) {
        		...
        		//found the node
        		if (this.isLeaf()) {
        			//if the element is a leaf remove it from the tree
        			System.out.println("parent = "+parent);
        			if (parent != null) {
        				System.out.println("pleft = "+parent.left);
        				System.out.println("pright = "+parent.right);
        				parent.removeChild(this);
        			}
        			debug("Is a leaf");
        			return this;
        		} else if (right == null && left != null) {
        			//if the node only has 1 child then add it to the parent
        			left.parent = parent;
        			if (parent != null) {
	            		if (parent.left == this) {
	            			parent.left = left;
            		...
            		debug("only has right child");
            		return this;
            	} else {
            		// swapping values of this node and the successor
            		AvlNode<K, V> next = successor(this);
            		debug("swapping with "+next);
            		debug("next children l ="+next.left+" r = "+next.right);
            		K tempKey = this.key;
            		...
            		next.key = tempKey;
            		next.value = tempVal;
            		return next.delete(next);
            	}	
        	}      	
        }
        
        public boolean removeChild(AvlNode<K, V> node) {
        	if (right != null) {
        		if (right.key == node.key && right.value == node.value) {
	        		right = null;
	        		rightHeight--;
	        		return true;
	        	}
        	...
        	if (left != null) {
	        	if (left.key == node.key && left.value == node.value) {
	        		left = null;
	        		leftHeight--;
	        		return true;
	        	}
    ...
    class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean equals(final Object other) {
            ...
            System.out.println("size i "+i+" : size j "+j);
            System.out.println("containsAll "+(((Collection<?>) other).containsAll(this)));
        ...
        public boolean remove(Object o) {
            ...
            if (p == null)
            	return false;
            if (!valEquals(p.getValue(), value))
            	return false;
            if (AvlGTree.this.remove(p.getKey()) != null)
            	return true;
            else 
            	return false;
        }
    ...
    final class SubMap<K, V> extends AbstractMap<K, V> implements
        ...
        class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
            ...
            public boolean remove(Object o) {
                if (AvlGTree.this.remove(o) != null)
                	return true;
                else 
                	return false;
            }
